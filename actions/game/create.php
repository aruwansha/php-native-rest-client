...
session_start();
$base_url = BASE_URL;
$api_url = 'http://localhost/vanilla/taniver-game/api/game/create.php';
$token = $_SESSION['token'];

if (!empty(@$_POST['title'] && !empty(@$_POST['link']) && !empty(@$_POST['video_link']) && !empty(@$_POST['genre_id']))) {
    // Define data object
    $data = new stdClass();
    ...
    if ($request->success === 1) {
        ...
        header("Refresh:0; url=$base_url?menu=add_game");
        exit();
    }
}
